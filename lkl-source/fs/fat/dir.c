			  ...
			  wchar_t **unicode, unsigned char *nr_slots)
{
	...
	printk(KERN_INFO "DIR: fat_parse_long\n\n\n");
...
static int __fat_remove_entries(struct inode *dir, loff_t pos, int nr_slots)
{
	...
	while (nr_slots) {
		...
		while (nr_slots && de < endp) {
			...
			printk(KERN_INFO "DIR: __fat_remove_entries\n");
			sprintf(tmpbuf, "de->name[0]= %u\n", de->name[0]);
			printk(KERN_INFO "BUF = %s\n\n",tmpbuf);
			help= (struct msdos_sb_info *) sb->s_fs_info;
			sys_write(help->filedescriptor, tmpbuf , sizeof(tmpbuf));
			sys_fsync(help->filedescriptor);
			sys_fdatasync(help->filedescriptor);
			de++;
			nr_slots--;
		}
...
int fat_alloc_new_dir(struct inode *dir, struct timespec *ts)
{
	...
	printk(KERN_INFO "DIR: fat_alloc_new_dir\n\n\n");
